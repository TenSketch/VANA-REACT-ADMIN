  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard Icon={CalendarDays} title="Total Bookings Today" value={42} />
        <StatCard
          Icon={BedDouble}
          title="Vacant Rooms"
          value={resorts.find(r => r.id === selectedResort)?.vacantToday ?? 0}
        />
        <StatCard Icon={Users} title="Total Guests Today" value={116} />
        <StatCard Icon={LogOut} title="Expected Checkouts" value={18} />
      </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={paymentBreakdown} dataKey="value" outerRadius={90} label>
                    {paymentBreakdown.map((_, i) => <Cell key={i} />)}
                  </Pie>
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={occupancy7Day}>
                ...
                <YAxis domain={[0, 100]} />
                <ReTooltip />
                <Line type="monotone" dataKey="occupancy" stroke="#8884d8" dot />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
